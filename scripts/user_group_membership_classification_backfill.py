...
if len(not_in_group_yet_user_ids) > 0:
    ...
    eras_cursor.execute("SELECT classification_id, event_time, session_time, project_id, user_id, workflow_id, created_at, updated_at, user_group_ids from classification_events WHERE user_id = ANY(%s) AND ((%s <> ALL(user_group_ids)) OR user_group_ids IS NULL)", (not_in_group_yet_user_ids, user_group_id))
